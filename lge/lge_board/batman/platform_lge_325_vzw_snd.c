/* lge/lge_board/batman/board_lge_325_snd.c
 *
...
#include "timpani_profile_8x60_lge_325_vzw.h"
...
/* define the value for Earjack Folder test, RX */
static struct adie_codec_action_unit ihs_stereo_acs_left_48KHz_osr256_actions[] =
#ifdef CONFIG_LGE_AUDIO_NO_NCP_MODE
	HEADSET_RX_L_STEREO_AB_LEG;
#else
	HEADSET_AB_CPLS_48000_OSR_256;
#endif
static struct adie_codec_hwsetting_entry ihs_stereo_acs_rx_left_settings[] = {
	{
		.freq_plan = 48000,
		.osr = 256,
		.actions = ihs_stereo_acs_left_48KHz_osr256_actions,
		.action_sz = ARRAY_SIZE(ihs_stereo_acs_left_48KHz_osr256_actions),
	}
};
static struct adie_codec_dev_profile ihs_stereo_acs_rx_left_profile = {
	.path_type = ADIE_CODEC_RX,
	.settings = ihs_stereo_acs_rx_left_settings,
	.setting_sz = ARRAY_SIZE(ihs_stereo_acs_rx_left_settings),
};
static struct snddev_icodec_data snddev_ihs_stereo_left_acs_rx_data = {
	.capability = (SNDDEV_CAP_RX | SNDDEV_CAP_VOICE),
	.name = "headset_acs_rx_l",
	.copp_id = 0,
	.profile = &ihs_stereo_acs_rx_left_profile,
	.channel_mode = 2,
	.default_sample_rate = 48000,
/*                                                          */
#ifdef CONFIG_LGE_AUDIO_AMP_WM9093
...
static struct platform_device msm_headset_stereo_left_acs_device = {
	.name = "snddev_icodec",
	.dev = { .platform_data = &snddev_ihs_stereo_left_acs_rx_data },
};

static struct adie_codec_action_unit ihs_stereo_acs_right_48KHz_osr256_actions[] =
#ifdef CONFIG_LGE_AUDIO_NO_NCP_MODE
	HEADSET_RX_R_STEREO_AB_LEG;
#else
	HEADSET_AB_CPLS_48000_OSR_256;
#endif
static struct adie_codec_hwsetting_entry ihs_stereo_acs_rx_right_settings[] = {
	{
		.freq_plan = 48000,
		.osr = 256,
		.actions = ihs_stereo_acs_right_48KHz_osr256_actions,
		.action_sz = ARRAY_SIZE(ihs_stereo_acs_right_48KHz_osr256_actions),
	}
};
static struct adie_codec_dev_profile ihs_stereo_acs_rx_right_profile = {
	.path_type = ADIE_CODEC_RX,
	.settings = ihs_stereo_acs_rx_right_settings,
	.setting_sz = ARRAY_SIZE(ihs_stereo_acs_rx_right_settings),
};
static struct snddev_icodec_data snddev_ihs_stereo_right_acs_rx_data = {
	.capability = (SNDDEV_CAP_RX | SNDDEV_CAP_VOICE),
	.name = "headset_acs_rx_r",
	.copp_id = 0,
	.profile = &ihs_stereo_acs_rx_right_profile,
	.channel_mode = 2,
	.default_sample_rate = 48000,
/*                                                          */
#ifdef CONFIG_LGE_AUDIO_AMP_WM9093
...
static struct platform_device msm_headset_stereo_right_acs_device = {
	.name = "snddev_icodec",
	.dev = { .platform_data = &snddev_ihs_stereo_right_acs_rx_data },
};
...
{
#ifdef CONFIG_LGE_AUDIO_AMP_WM9093
    const char* AudioSubsystemPath  = "/data/wm9093_lge_325_atnt.txt";
    const char* AudioSubsystemPath2 = "/etc/wm9093_lge_325_atnt.txt";
#else
...
#ifdef CONFIG_LGE_AUDIO_AMP_WM9093
        if (token) /* mbhyun.kim 2013.01.22 : WBT ID 2101368376 */
...
int get_wm9093_cal_db(char *data_array)
{
    ...
    /* Header */
    sprintf(str, "WM9093 for %s (%s)", "DS1006", __DATE__);
...
{
    const char* QTR8615LPath  = "/data/qtr8615l_lge_325_atnt.txt";
    const char* QTR8615LPath2 = "/etc/qtr8615l_lge_325_atnt.txt";
    struct file *filp;
    ssize_t read_size = 0;
    char data_array[1000];
    char *token = NULL;
...
int get_qtr8615l_cal_db(char *data_array, int *data_len)
{
    ...
    /* Header */
    sprintf(str, "QTR8615L for %s (%s)", "DS1006", __DATE__);    
    if(buff_len + strlen(str) + 4 >= ARRAY_SIZE(str)) goto END_OF_CODES;
    memcpy(&(data_array[buff_len]), str, strlen(str));
    buff_len +=  strlen(str);
    data_array[buff_len++] = '\n';
...
static struct platform_device *snd_devices_lge_325_board[] __initdata = {
...
//                                                                    
  ...
  &msm_headset_mic_record_device,
	...
	&msm_headset_stereo_left_acs_device, // EAR ACS_RX_L
	&msm_headset_stereo_right_acs_device, // EAR ACS_RX_R
};
...
void __init msm_snddev_init(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(snd_devices_lge_325_board); i++)
		snd_devices_lge_325_board[i]->id = dev_id++;

	platform_add_devices(snd_devices_lge_325_board, ARRAY_SIZE(snd_devices_lge_325_board));
