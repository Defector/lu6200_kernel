/* lge/lge_board/batman/platform_lge_325_bt.c
...
static int batman_bluetooth_toggle_radio(void *data, bool state)
{
	int ret;
	int (*power_control)(int enable);

	 printk(KERN_ERR "[LG_BTUI] %s : Called batman_bluetooth_toggle_radio",__func__);  	 	
...
static struct bluetooth_platform_data batman_bluetooth_data = {
	.bluetooth_power = batman_bluetooth_power,
	.bluetooth_toggle_radio = batman_bluetooth_toggle_radio,
};
...
#if 1 //i'm not sure why phone reset when bt toggle is added to  platform  [BT_TOGGLE]
	.dev = {
		.platform_data = &batman_bluetooth_data,	
#else
	.dev = {
		.platform_data = &batman_bluetooth_power,
#endif
	},		
};
...
static struct bluesleep_platform_data batman_bluesleep_data = {
...
static struct platform_device msm_bluesleep_device = {
	...
	.dev = {
		.platform_data = &batman_bluesleep_data,
	},	
};
