...
#define CUSTOMER_ID_REG					(ts->common_dsc.query_base+2)	      /* Customer ID */
...
#define FLASH_QUERY_REG					(ts->flash_dsc.query_base)
#define FLASH_DATA_REG						(ts->flash_dsc.data_base)
	...
	if (unlikely(touch_i2c_read(ts->client, CONFIG_ID_REG,
		...
		if (lge_bd_rev >= LGE_REV_E){
	      ...
	      if (vs950_need_touch_downlaod){
			fw_info->fw_start = (unsigned char *)&SynaFirmware[3][0];
			fw_info->fw_size = sizeof(SynaFirmware[3]);
		} else {
			fw_info->fw_start = (unsigned char *)&SynaFirmware[0][0];
			fw_info->fw_size = sizeof(SynaFirmware[0]);
			}
...
int synaptics_ts_init(struct i2c_client* client, struct touch_fw_info* fw_info)
{
	...
	/* Large Object Suppression Off */
	if (unlikely(touch_i2c_write_byte(client, TWO_D_REPORTING_MODE+41, 0x4B) < 0)) {
		TOUCH_ERR_MSG("Palm Suppression Off command write fail\n");
		return -EIO;
	}
	TOUCH_INFO_MSG("Palm Suppression Off\n");
  ...
  if(vs950_need_touch_downlaod){ // New ITO
		if (unlikely(synaptics_ts_page_data_write_byte(client, 0x02, BUTTON_CONTROL_REG+11, 0x53) < 0)) {
			TOUCH_ERR_MSG("PAGE_SELECT_REG write fail\n");
			return -EIO;
		}
		if (unlikely(synaptics_ts_page_data_write_byte(client, 0x02, BUTTON_CONTROL_REG+12, 0x5D) < 0)) {
			TOUCH_ERR_MSG("PAGE_SELECT_REG write fail\n");
			return -EIO;
		}
		if (unlikely(synaptics_ts_page_data_write_byte(client, 0x02, BUTTON_CONTROL_REG+13, 0x61) < 0)) {
			TOUCH_ERR_MSG("PAGE_SELECT_REG write fail\n");
			return -EIO;
		}
		if (unlikely(synaptics_ts_page_data_write_byte(client, 0x02, BUTTON_CONTROL_REG+14, 0x61) < 0)) {
			TOUCH_ERR_MSG("PAGE_SELECT_REG write fail\n");
			return -EIO;
		}
	} else {
		if (unlikely(synaptics_ts_page_data_write_byte(client, 0x02, BUTTON_CONTROL_REG+11, 0x58) < 0)) {
			TOUCH_ERR_MSG("PAGE_SELECT_REG write fail\n");
			return -EIO;
		}
		if (unlikely(synaptics_ts_page_data_write_byte(client, 0x02, BUTTON_CONTROL_REG+12, 0x62) < 0)) {
			TOUCH_ERR_MSG("PAGE_SELECT_REG write fail\n");
			return -EIO;
		}
		if (unlikely(synaptics_ts_page_data_write_byte(client, 0x02, BUTTON_CONTROL_REG+13, 0x66) < 0)) {
			TOUCH_ERR_MSG("PAGE_SELECT_REG write fail\n");
			return -EIO;
		}
		if (unlikely(synaptics_ts_page_data_write_byte(client, 0x02, BUTTON_CONTROL_REG+14, 0x66) < 0)) {
			TOUCH_ERR_MSG("PAGE_SELECT_REG write fail\n");
			return -EIO;
		}
	} // Old ITO
...
int get_customer(struct synaptics_ts_data* ts)
{
	...
	if (unlikely(touch_i2c_read(ts->client, FLASH_QUERY_REG, 2, &uData[0]) < 0)) {
		TOUCH_ERR_MSG("FLASH_QUERY_REG read fail\n");
		return -EIO;
	}

	if (unlikely(touch_i2c_write(ts->client, FLASH_DATA_REG+2, 2, &uData[0]) < 0)) {
		TOUCH_ERR_MSG("FLASH_DATA_REG + 2 write fail\n");
		return -EIO;
	}

	uData[0] = 0x0f;

	if (unlikely(touch_i2c_write(ts->client, FLASH_DATA_REG+18, 1, &uData[0]) < 0)) {
		TOUCH_ERR_MSG("FLASH_DATA_REG + 18 write fail\n");
		return -EIO;
	}

	if (unlikely(touch_i2c_read(ts->client, FLASH_QUERY_REG+3, 2 , &uData[0]) < 0)) {
		TOUCH_ERR_MSG("FLASH_QUERY_REG+3 read fail\n");
		return -EIO;
	}

	if (unlikely(touch_i2c_read(ts->client, CUSTOMER_ID_REG, 2 , &uData[0]) < 0)) {
		TOUCH_ERR_MSG("CUSTOMER_ID_REG read fail\n");
		return -EIO;
	}

	if(uData[0] == 0x01){
		customer_family  = 1;
			//TOUCH_INFO_MSG("customer_family= %x\n", ts->customer_family);
	} else {
		customer_family  = 0;
	}
...
int synaptics_ts_customer(struct i2c_client* client)
{
	struct synaptics_ts_data* ts =
			(struct synaptics_ts_data*)get_touch_handle(client);
...
if (unlikely(touch_i2c_write_byte(client, PAGE_SELECT_REG, 0x01) < 0)) {
		...
		case BASELINE_REBASE:
			...
			break;
		case PALM_SUPPRESSION_ON:
			/* Large Object Suppression On */
			if (likely(ts->finger_dsc.id != 0)) {
				if (unlikely(touch_i2c_write_byte(client, TWO_D_REPORTING_MODE+41, 0xCB) < 0)) {
					TOUCH_ERR_MSG("Palm Suppression On command write fail\n");
					return -EIO;
				}
			}
			if (unlikely(touch_debug_mask & DEBUG_GHOST))
			TOUCH_INFO_MSG("PALM_SUPPRESSION_ON  ~~~~~~~~\n");
			break;
		default:
			break;
		}
